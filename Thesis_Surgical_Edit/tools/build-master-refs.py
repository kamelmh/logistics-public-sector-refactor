...
def parse_entry(line):
    """Parse a single bibliography line like '1. **Author.** (year). Title...'"""
...
def classify_entry(num, author, year):
    """Determine if entry is academic (English) or Arabic regulatory."""
...
def make_key(author, year, title_full="", number=0):
    """Generate canonical key: 'chopra-meindl-2019'"""
...
def extract_all_entries(md_path):
    """Read .md and extract all 56 bib entries with sections."""
...
def build_pdf_mapping(entries):
    """Map bib entries to PDFs in refs/ based on semester maps above."""
    pdf_map = {}
...
def clean_old_naming():
    """Dedup Old_Naming: check if each PDF exists in semester folders, if not move it."""
