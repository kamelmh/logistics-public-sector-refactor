...
Public Sub CheckWorkbookAccess()
    If IsWorkbookLocked() Then
        ...
        response = MsgBox("ATTENTION: Ce fichier est dj ouvert par: " & owner & vbCrLf & _
                         vbCrLf & _
                         "L'ouverture en mode lecture seule est recommande pour viter" & vbCrLf & _
                         "la corruption des donnes. Continuer en mode lecture/criture?", _
                         vbYesNo + vbExclamation, "Accs concurrent dtect")
...
Public Sub BatchExportPDFs(ByVal startDate As Date, ByVal endDate As Date)
    ...
    If docRefs.Count = 0 Then
        MsgBox "Aucun document trouv pour la priode slectionne.", vbInformation
        Exit Sub
    End If
...
Public Sub BulkImportMovements(ByVal importSheetName As String)
    ...
    If lastRow < 2 Then
        MsgBox "Aucune donne  importer.", vbInformation
        Exit Sub
    End If
