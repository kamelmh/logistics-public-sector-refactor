...
'   Col I (9) : THIRD_PARTY  (new -- add ws.Cells(nextRow,9) in Database)
'   Col L (12): NOTES
'==============================================================================
...
Public Sub ExportTransactionToPDF(ByVal docRef As String)
    ...
    If Not SheetExists("TEMPLATE_BON") Then
        MsgBox "ERREUR: Feuille TEMPLATE_BON introuvable." & vbCrLf & _
               "Lancez mod_SheetSetup.CreateAllSheets() d'abord.", _
               vbCritical, "ACADEMIX v13"
        Exit Sub
    End If
    ...
    ' 2. Populate template from transaction data (NEW -- was missing)
...
Private Function PopulateTemplateBon(ByVal docRef    As String, _
    ...
    ' Fallback to mod_Database.bas hardcoded positions if headers missing
    If colDate  = 0 Then colDate  = 1
    If colCode  = 0 Then colCode  = 2
    If colDesig = 0 Then colDesig = 3
    If colType  = 0 Then colType  = 4
    If colQte   = 0 Then colQte   = 5
    If colRef   = 0 Then colRef   = 7
