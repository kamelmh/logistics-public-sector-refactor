@echo off
title OpenCode Launcher v3.6 ^| CrossFlow ^| Academix v13.2
:: ==============================================================
...
set "HANDOFF_DIR=%PROJECT_ROOT%\.crossflow"
...
echo   sync       Incremental context sync (CocoIndex pattern)
...
:: ==============================================================
:sync
title OpenCode [SYNC] - %SESSION_NAME%
echo   === INCREMENTAL CONTEXT SYNC ===
%PWSH% -File "%PROJECT_ROOT%\scripts\context-sync.ps1"
pause
goto :end
...
echo   1) Search Memory
echo   2) Capture Observation
echo   3) View Detail
set /p MEM_CHOICE="Select [1-3]: "
if "%MEM_CHOICE%"=="1" (
    set /p Q="Query: "
    %PWSH% -Command "& '%PROJECT_ROOT%\scripts\mem-util.ps1' -Action search -Query '%Q%'"
)
if "%MEM_CHOICE%"=="2" (
    set /p T="Tag: "
    set /p O="Observation: "
    %PWSH% -Command "& '%PROJECT_ROOT%\scripts\mem-util.ps1' -Action capture -Tag '%T%' -Observation '%O%'"
)
if "%MEM_CHOICE%"=="3" (
    set /p I="Observation ID: "
    %PWSH% -Command "& '%PROJECT_ROOT%\scripts\mem-util.ps1' -Action detail -Id '%I%'"
)
pause
goto :end

:sandbox
title OpenCode [SANDBOX] - %SESSION_NAME%
echo   === WORKTREE SANDBOX ITERATION ===
echo   Usage: powershell -File scripts\sandbox-iterate.ps1 -BranchName "name" -IterationLogic { ... } -MetricCommand "..."
echo.
echo   This mode is best used via the CLI by passing the script directly.
echo   Example: %PWSH% -File "%PROJECT_ROOT%\scripts\sandbox-iterate.ps1" -BranchName "test" -IterationLogic { ... } -MetricCommand "& 'vbe-auto\verify.ps1'"
pause
goto :end
